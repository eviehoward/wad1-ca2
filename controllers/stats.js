...
const stats = {
  createView(request, response) {
    ...
    if (loggedInUser) {
      ...
      let userLongest = userCollections.length > 0 ? Math.max(...userCollections.map(collection => collection.sets.length)) : 0;
      ...
      let userShortest = userCollections.length > 0 ? Math.min(...userCollections.map(collection => collection.sets.length)) : 0;
